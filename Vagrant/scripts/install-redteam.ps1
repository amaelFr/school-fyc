...
# # Download and install Rubeus
Write-Host "$('[{0:HH:mm}]' -f (Get-Date)) Downloading Rubeus"

# # $tag = (Invoke-WebRequest "https://api.github.com/repos/GhostPack/Rubeus/releases" -UseBasicParsing | ConvertFrom-Json)[0].tag_name
# # $rubeusDownloadUrl = "https://github.com/GhostPack/Rubeus/archive/refs/tags/$tag.zip"
# # $rubeusRepoPath = "C:\Users\vagrant\AppData\Local\Temp\rubus_$tag"
# If (-not (Test-Path "C:\Tools\Rubeus")) {
#   # Invoke-WebRequest -Uri "$rubeusDownloadUrl" -OutFile "$rubeusRepoPath.zip"
#   # Expand-Archive -path "$rubeusRepoPath.zip" -destinationpath $rubeusRepoPath -Force
  
#   # C:\Windows\Microsoft.NET\Framework\v4.0.30319\MSBuild.exe $rubeusRepoPath\Rubeus-$tag\Rubeus\Rubeus.csproj
  
  New-Item -Path "C:\Tools" -Name "Rubeus" -ItemType "directory" -Force
  [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
  Invoke-WebRequest -Uri "https://github.com/r3motecontrol/Ghostpack-CompiledBinaries/raw/master/Rubeus.exe" -OutFile "C:\Tools\Rubeus\Rubeus.exe"

# } Else {
#   Write-Host "$('[{0:HH:mm}]' -f (Get-Date)) Rubeus was already installed. Moving On."
# }
